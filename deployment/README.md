...
To give other developers access to the server:

1.  **Ask them for their Public SSH Key** (usually `~/.ssh/id_ed25519.pub` or `~/.ssh/id_rsa.pub`).
2.  **SSH into the server**:
    ```bash
    ssh root@<DROPLET_IP>
    ```
3.  **Add their key** to the `authorized_keys` file:
    ```bash
    nano ~/.ssh/authorized_keys
    ```
4.  **Paste their public key** on a new line, save (Ctrl+O), and exit (Ctrl+X).

They can now SSH into the server using their private key.
